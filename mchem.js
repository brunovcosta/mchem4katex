...
var texify = {
	types: {
		'chemfive': function (buf) {
			...
			if (buf.b || buf.p) {
				...
				//res += "{\\vphantom{X}}";
				//res += "^{{\\vphantom{2}}\\llap{"+(buf.b||"")+"}}";
				//res += "_{\\vphantom{2}\\llap{{"+(buf.p||"")+"}}}";
				//res += "\\mskip-0mu ";
			}
			//
			// o
		...
		'arrow': function (buf) {
			...
			if (buf.rd || buf.rq) {
				if (buf.r === "<=>>"  ||  buf.r === "<<=>"  ||  buf.r === "<-->") {
					...
					arrow = "\\tiny{\\begin{matrix} \\footnotesize\\ "+buf.rd+"\\\\ \\Large \\long"+arrow+"\\\\\\\\ \\end{matrix}}";//@brunovcosta: my alternative for \xrightarrow (not suported in KaTeX)
					arrow = " \ {"+arrow+"}\  ";
				}
			} else {
				arrow = " \ {\\"+arrow+"}\  ";
	...
	entities: {
		...
		'1000 separator': "\\  ",
		'commaDecimal': "{,}",
		'comma enumeration L': "{{0}}\\ ",
		'comma enumeration M': "{{0}}\  ",
		'comma enumeration S': "{{0}}\\ ",
		'hyphen': "\\text{-}",
